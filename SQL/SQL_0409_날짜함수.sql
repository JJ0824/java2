...
select now(),
datediff('2025-12-20', now()), -- 620 앞쪽에서 뒤쪽을 뺌
datediff(now(), '2025-12-20'),
timestampdiff(year, now(), '2025-12-20'), -- 연도차이 월,일도 가능 
...
select now(),
adddate(now(), 50), -- 현재 날짜에서 50일을 더함 
adddate(now(), -50),
adddate(now(), interval 50 day),
adddate(now(), interval 50 month), -- 50 개월 뒤 
...
-- monthname 월을 영문으로
-- weekday 요일을 정수로 (MySql 월요일 0 ~ 일요일 6)
select now(),
last_day(now()),
dayofyear(now()),
monthname(now()),
