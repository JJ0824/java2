...
-- 재고 검사
-- select 주문세부.제품번호, 주문세부.주문번호, 주문세부.주문수량, 제품.재고
-- from 주문세부
-- join 제품 on 주문세부.제품번호 = 제품.제품번호
-- where 제품.재고 < 50;
...
-- 주문기록 검사
-- select 고객.고객번호, 고객.고객회사명, 주문.고객번호 
-- from 고객
-- left join 주문 on 고객.고객번호 = 주문.고객번호
-- where 주문.고객번호 is null;

#4 '2022-02-01'에서 3개월 내 주문이 있는 사원의 사원번호, 이름, 직위 조회
select 사원.사원번호, 사원.이름, 사원.직위
from 사원
join 주문 on 사원.사원번호 = 주문.사원번호
where (주문.주문일 >= ('2022-02-01' - interval 3 month)) and (주문.주문일 <= ('2022-02-01' + interval 3 month));

-- 주문일 검사
-- select 사원.사원번호, 사원.이름, 사원.직위, 주문.주문일
-- from 사원
-- join 주문 on 사원.사원번호 = 주문.사원번호
-- where (주문.주문일 >= ('2022-02-01' - interval 3 month)) and (주문.주문일 <= ('2022-02-01' + interval 3 month));

#5 고객별 주문수를 계산하여 고객회사명, 주문수 조회
select 고객.고객회사명, count(*) as 주문건수
from 고객
left join 주문 on 고객.고객번호 = 주문.고객번호
...
#7 제품테이블에 있는 제품 중 단가(주문세부 테이블)가 가장 높은 제품명
select 제품.제품명, max(주문세부.단가) as 최고단가
from 제품
join 주문세부 on 제품.제품번호 = 주문세부.제품번호
group by 제품.제품명
order by 최고단가 desc
...
#8 제품테이블에 있는 제품단가가 가장 높은 제품의 주문수량합
select 제품.제품번호, 제품.제품명, sum(주문세부.주문수량) as 주문수량합
from 제품
join 주문세부 on 제품.제품번호 = 주문세부.제품번호
join (select max(단가) as 최고단가 from 제품) as 최고단가정보 
    on 최고단가정보.최고단가 = 제품.단가
group by 제품.제품번호, 제품.제품명;

#9 '아이스크림'제품의 주문수량합
select 주문세부.제품번호, sum(주문세부.주문수량) as 주문수량합
from 주문세부
join 제품 on 주문세부.제품번호 = 제품.제품번호
...
#10 '서울특별시'고객들에 대한 주문년도별 주문건수
select year(주문.주문일) as 주문년도, count(*) as 주문건수
from 주문
join 고객 on 주문.고객번호 = 고객.고객번호
where 고객.도시 = '서울특별시'
group by 주문년도
