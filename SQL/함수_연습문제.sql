...
select 고객회사명, 
		concat('**', replace(고객회사명, left(고객회사명, 2), '')) as 고객회사명2, 
...
-- as 뒤에 ''을 써서 표기하는게 좀 더 명시적임, 대,소문자를 섞어서 표기하는 게 중요하다 생각한다면, 사이에 공백이 들어간다면 꼭 ''안에 넣어야 함. ``에 넣어도 됨		
from 주문세부;
...
select 담당자명,
	고객회사명,
	도시,
-- 	if ((right(도시,3) = '특별시') or (right(도시,3) = '광역시'), '대도시', '도시')
