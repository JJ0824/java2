...
-- substr 문자열의 몇 번째부터 몇 개
select left('SQL 공부', 3),
		right('SQL 공부', 2),
		substr('SQL 공부', 3, 4),
		substr('SQL 공부', 5);
-- substring_index
-- 구분자를 만날 때까지 문자열을 잘라냄, 인덱스는 몇 번째 구분자를 만날 때까지 자를지 결정	
-- 만약 인덱스가 음수 값이면, 오른쪽에서 자름
...
-- lpad 채우기, 지정한 길이 내에서 문자열을 제외한 나머지를 주어진 문자로 왼쪽부터 채움
-- rpad 채우기, 지정한 길이 내에서 문자열을 제외한 나머지를 주어진 문자로 오른쪽부터 채움
